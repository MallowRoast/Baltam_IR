...
> 如果后续想认真设计“高层结构化 IR -> 低层 CFG / machine IR”的分层，MLIR 的 `scf` dialect 是非常值得参考的现成范例。
...
- 如果后面要认真做 `high-level IR -> typed SSA` 的第二层 `IR`，这套材料很适合作为“较低层 `IR` 长什么样”的对照物
