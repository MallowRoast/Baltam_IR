...
>
> `global` 不应设计成 per-call frame slot。它的真实存储属于 session/global table，跨函数共享；
> 但源码中的 global 名字仍应有静态 `SlotId`，该 slot 持有的 `ba_obj_ptr` 与
> `GlobalRegistry::values[name]` 指向同一个变量地址。
...
PersistentSlot
  ...
  SlotId 稳定，并作为函数私有 persistent value table 的 key

DynamicEnv
  eval / script / load / assignin 动态创建的 workspace 名字

GlobalRegistryValue
  当前 CodeUnit 内静态 global slot 接入的 session 共享变量
```
...
`FunctionFrame::slot_values[offset]`，而应映射到当前函数 code object 的 persistent value table。

`global` 是 shared binding：多个声明了同名 `global x` 的 workspace 会读写同一个全局 value，
其他函数调用可能改变它。它不属于某个函数 frame 的 per-call storage，但仍应使用 `SlotId`
表达源码中的静态变量身份；这个 slot 连接到 `GlobalRegistry::values[name]`，不是 frame offset。
...
Persistent slot:
  SlotId -> CodeObject::persistent.values[SlotId]

Global slot:
  SlotId -> ba_obj_ptr -> InterpreterContext::globals.values[name]
```

三者都可以在函数内静态确定 slot 身份，但生命周期和存储位置不同。
...
```text
load_global %slot_x
  读取 %slot_x 指向的 session/global table value

store_global %slot_x, %v
  写入 %slot_x 指向的 session/global table value

load_persistent %slot_p
  读取当前 FunctionUnit 私有 persistent value table 中 %slot_p 对应的值

store_persistent %slot_p, %v
  写入当前 FunctionUnit 私有 persistent value table 中 %slot_p 对应的值
```

这里的关键区别是：global slot 连接到 session 共享 `GlobalRegistry::values[name]`，persistent
slot 用 `SlotId` 定位当前函数内部的 persistent value table。二者都使用 `SlotId` 表达源码变量身份，但都不是
普通 per-call frame slot。
...
`persistent` 的 slot 位置也能静态确定，但它指向的是函数私有 persistent value table：

```text
code_object.persistent.values[slot_id]
```

递归或再次调用同一函数时，读写的是同一份 value entry。
...
## 6. 为什么 global 有 SlotId 但不是 frame slot

`global` 的存储是 session 共享 value：

```text
global_registry.values[name]
```

同名 global 可能被多个函数、脚本或命令行上下文共享修改。它不属于某个 `FunctionUnit` 的
per-call frame storage，也没有当前函数内可独立拥有的 frame slot value。

runtime 初始化 Frame 时把 `SlotId` 对应的 slot value 连接到全局表中的同名 value：

```text
IR %slot_g -> GlobalRegistry::values[name]
```

这个地址指向全局变量表中的 value，不是当前函数独立拥有的 per-call frame value。`SlotId` 只是
当前 code unit 内的静态变量身份。
...
- runtime 初始化 global slot 和 persistent value table
...
建议让 global 和 persistent 都进入 `FunctionUnit` 或 `CodeUnit` 的 slot 表，并保留声明源码位置：
...
SlotInfo / declaration metadata
  slot        : Slot::Global | Slot::Persistent
  name        : InternedString
...
- `Slot::Global` 可以出现在 `FunctionUnit`，也可以在脚本语义允许时出现在 `ScriptUnit`。
- 同一程序点上，同名不能同时绑定为普通 local、global 和 persistent。
- 同一 code unit 中，`clear` 之后的后续语句可能让同一个源码名字进入新的 binding state；
  例如 `persistent p; clear p; p = 1` 中，最后的 `p` 不再写 persistent value table，而是普通 local
  绑定。verifier 不应简单按全 code unit 禁止这种“跨 clear 的绑定切换”。
- 同一 code unit 中，同名 global slot 应去重，重复声明可以 warning 或按 Matlab 行为兼容。
- `persistent_slots` 记录当前函数的 persistent slot 集合；运行时直接用 `SlotId` 作为
  `CodeObject::persistent.values` 的 key，不需要 persistent offset。
...
```text
基础 lowering:
  global / persistent 声明 -> 创建 Slot::Global / Slot::Persistent 并记录声明事实
  global / persistent 名字访问 -> 保留 workspace/binding 语义

binding specialization:
  如果证明当前程序点 binding 仍为 GlobalRegistryValue
    -> load_global/store_global
...
```text
%g0 = load_global %slot_g
%one = const 1
%g1 = call plus(%g0, %one)
store_global %slot_g, %g1
%g2 = load_global %slot_g
store_slot %slot_y, %g2
```

基础 lowering 阶段，`g` 进入当前 code unit 的 slot 表，类型为 `Slot::Global`。若后续分析证明从
`global g` 到这些访问之间没有相关 binding barrier，则可把 workspace/binding 访问特化成上面的
`load_global/store_global`。
...
workspace/binding 语义。若后续分析证明当前程序点的 `n` 仍绑定到 persistent value table，则可特化成
...
应把当前 target workspace 中的 `g` 绑定到 global value，之后脚本对 `g` 的读写可以有两种实现：

1. 保守方案：仍用 `ScriptVar` slot 访问或 generic workspace binding access，workspace lookup
   命中 `GlobalRegistry::values[name]`。
...
%1 = load_global %slot_g
store_global %slot_g, %2
...
- global 和 persistent 都有 slot
- 它们的读写指令不是普通 `load_slot/store_slot`，因为底层存储不在 per-call frame 中
...
```text
IR %slot_g   -> GlobalRegistry::values[name]
IR %slot_n   -> CodeObject::persistent.values[%slot_n]
```

解释器直接按 IR 节点执行：

```text
LoadGlobalInst      -> GlobalRegistry::values[name]
StoreGlobalInst     -> GlobalRegistry::values[name]
LoadPersistentInst  -> CodeObject::persistent.values[slot_id]
StorePersistentInst -> CodeObject::persistent.values[slot_id]
```

其中：

- global slot 指向 session global table value
- persistent slot id 指向当前 function code object 的 persistent value entry
...
因此，persistent 是 slot-addressed function-private state；global 是 slot-addressed shared state。
二者区别在于 slot 指向的底层 cell 所属不同。
...
`clear g` 会解除当前 workspace 中 `g -> GlobalRegistry::values["g"]` 的绑定。后续 `g = 20` 不能继续使用
`store_global %slot_g`，除非后面再次出现 `global g` 重新建立绑定。
...
```text
clear_binding %slot_g
  invalidates later load_global/store_global for %slot_g in the current binding context
...
- `load_global/store_global`：显式 global value 访问，使用 `SlotId`
- `load_persistent/store_persistent`：当前函数 persistent value table 访问，使用 `SlotId`
...
value 访问或 persistent slot 访问的空间。
