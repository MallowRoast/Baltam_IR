...
void test_execute_test43() {
    Module ssa_module = build_untyped_ssa_module_for_test_script("test43");
    Function& entry_function = entry_function_or_fail(ssa_module, "test43");
    ...
    expect(result.outputs.size() == 1, "test43 should expose a single return value.");
    expect(result.outputs.front().type == interpreter::Value::Concrete,
           "test43 return value should be concrete.");
    expect(result.outputs.front().object != nullptr, "test43 return object should not be null.");
    ...
    std::cout << "test43 outputs: ret=" << ret << ", arg2=" << arg2 << ", c=" << c
              << ", d=" << d << ", e=" << e << '\n';

    expect_near(ret, 0.0, 1e-12, "test43 return value mismatch.");
    expect_near(arg2, 3.0, 1e-12, "test43 final arg2 mismatch.");
    expect_near(c, 18.0, 1e-12, "test43 final c mismatch.");
    expect_near(d, 1.0 + std::sqrt(3.0), 1e-12, "test43 final d mismatch.");
    expect_near(e, 5.0, 1e-12, "test43 final e mismatch.");
}

}  // namespace

int main() {
    return run_runtime_test("test_test43", [] {
        test_execute_test43();
    });
}
