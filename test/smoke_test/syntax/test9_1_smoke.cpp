...
int main() {
    try {
        const baltam::IRBuildResult ir = baltam::smoke_test::build_ir(TEST9_1_MFILE_PATH);
        baltam::smoke_test::require_ir_is_complete(ir);
        baltam::smoke_test::require(ir.mfile->is_function_file(), "test9_1 应为函数文件");
        baltam::smoke_test::require(
            ir.mfile->code_units.size() == 2,
            "test9_1 文件应包含主函数和一个 local function");

        const baltam::FunctionUnit* function =
            baltam::find_function(*ir.mfile, "test9_1");
