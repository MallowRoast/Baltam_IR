...
std::vector<const GlobalDeclInst*> find_global_decls(const CodeUnit& unit) {
    std::vector<const GlobalDeclInst*> decls;
    for (const auto& block_ptr : unit.basic_blocks) {
        for (const auto& inst_ptr : block_ptr->instructions) {
            if (inst_ptr != nullptr && inst_ptr->type() == Instruction::GlobalDecl) {
                decls.push_back(static_cast<const GlobalDeclInst*>(inst_ptr.get()));
            }
        }
    }
    return decls;
...
void require_declares_slots(
    ...
    const char* message) {
    const std::vector<const GlobalDeclInst*> decls = find_global_decls(unit);
    smoke_test::require(decls.size() == 1, message);
    smoke_test::require(decls.front()->slots.size() == names.size(), message);

    for (std::string_view name : names) {
        const SlotInfo* slot = require_slot(unit, name, SlotTag::Global, message);
        smoke_test::require(
...
void verify_main_function(const FunctionUnit& function) {
    smoke_test::require(function.param_slots.size() == 1, "test9 应声明一个输入参数");
    smoke_test::require(function.return_slots.size() == 1, "test9 应声明一个返回值");
    require_slot(function, "a", SlotTag::Arg, "test9 参数 a 应为 Arg slot");
    require_slot(function, "z", SlotTag::Ret, "test9 返回 z 应为 Ret slot");
    require_declares_slots(
        function,
        {"x", "y"},
        "test9 主函数应声明两个 global slot");

    const Slot x = require_slot(function, "x", SlotTag::Global, "x 应为 Global slot")->slot;
    const Slot y = require_slot(function, "y", SlotTag::Global, "y 应为 Global slot")->slot;
    smoke_test::require(
...
void verify_local_function(const FunctionUnit& function) {
    ...
    const Slot x = require_slot(function, "x", SlotTag::Global, "f x 应为 Global slot")->slot;
    smoke_test::require(
        count_loads(function, x) == 1,
        "f 应从 Global slot 读取 x");
}
...
int main() {
    try {
        ...
        std::cerr << "test9_smoke 失败: " << ex.what() << '\n';
        return 1;
    }
