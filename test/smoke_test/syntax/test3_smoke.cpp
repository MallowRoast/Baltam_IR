...
void verify_while_cfg(const CodeUnit& unit) {
    ...
    smoke_test::require(unit.basic_blocks.size() == 4, "test3 应生成 entry + 3 个 while 基本块");
...
void verify_core_focus(const IRBuildResult& result) {
    ...
    smoke_test::require(
        smoke_test::count_instructions(*entry, Instruction::StoreSlot) == 4,
...
int main() {
    try {
        const baltam::IRBuildResult ir = baltam::smoke_test::build_ir(TEST3_MFILE_PATH);
